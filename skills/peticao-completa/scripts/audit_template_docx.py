#!/usr/bin/env python3
"""Audit the peticao-completa DOCX template formatting."""
...
def xml_checks(path: Path) -> list[tuple[str, bool]]:
    ...
    with ZipFile(path) as package:
        ...
        checks.append(("header logo optional", True if not os.environ.get("PETICAO_LOGO_REQUIRED") else header_has_logo))
...
def audit(path: Path = DEFAULT_INPUT) -> int:
    ...
    checks.extend(
        [
            ("signature name present", SIGNATURE_NAME in all_text),
            ("signature OAB present", SIGNATURE_OAB in all_text),
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Audit the peticao-completa DOCX template.")
