#!/usr/bin/env python3
"""Generate the deterministic DOCX template for the peticao-completa skill."""
...
LOGO = Path(os.environ["PETICAO_LOGO_PATH"]) if os.environ.get("PETICAO_LOGO_PATH") else None
SIGNATURE_NAME = os.environ.get("PETICAO_SIGNATURE_NAME", "[NOME DO ADVOGADO]")
SIGNATURE_OAB = os.environ.get("PETICAO_SIGNATURE_OAB", "[OAB/UF]")
FOOTER_LINES = tuple(
    line.strip()
    for line in os.environ.get(
        "PETICAO_FOOTER_LINES",
        "[ENDERECO DO ESCRITORIO]|[E-MAIL]|[TELEFONE]",
    ).split("|")
    if line.strip()
)
...
def build_template(output: Path = DEFAULT_OUTPUT) -> Path:
    ...
    add_paragraph(doc)
    add_paragraph(doc, SIGNATURE_NAME, "Peticao Assinatura Nome")
    add_paragraph(doc, SIGNATURE_OAB, "Peticao Assinatura OAB")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Generate the peticao-completa DOCX template.")
